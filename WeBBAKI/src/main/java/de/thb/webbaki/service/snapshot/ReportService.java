...
public class ReportService {
    ...
    public Report getCompanyReport(Snapshot snapshot, String username) {
        return reportRepository.findBySnapshotAndUser_Username(snapshot, username);
    }

    public Report getBranchReport(Snapshot snapshot, Branch branch) {
        return reportRepository.findBySnapshotAndBranch(snapshot, branch);
    }

    public Report getSectorReport(Snapshot snapshot, Sector sector) {
        return reportRepository.findBySnapshotAndSector(snapshot, sector);
    }

    public Report getNationalReport(Snapshot snapshot) {
        return reportRepository.findBySnapshotAndUserIsNullAndBranchIsNullAndSectorIsNull(snapshot);
    }
    ...
    public void createCompanyReports(Snapshot snapshot, Map<Branch, Map<Scenario, List<ReportScenario>>> branchMapOftReportScenarioListMaps, Map<Sector, Map<Scenario, List<ReportScenario>>> sectorMapOftReportScenarioListMaps) {
        ...
        for (User user : userService.getAllUsers()) {
            ...
            // only create process if not all are empty
            if(companyReportScenarios.stream().mapToInt(ReportScenario::getNumberOfValues).sum() <= 0)
                continue;
     ...
     */
    public MappingReport getMappingReportByReportFocus(ReportFocus reportFocus, String username, Snapshot snapshot) throws
            UnknownReportFocusException {
     ...
     * @throws IOException All styles in the html have to be inline.
     *                     Not closed elements like <br> and <link> have to be closed like this
     *                     <br></br> and <link></link>
     */
    public void generatePdfFromHtml(String html, String baseUrl, OutputStream outputStream) throws
